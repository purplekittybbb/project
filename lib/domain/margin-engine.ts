...
import type { Transaction } from "./canonical";
...
/**
 * "What if I dropped this SKU" simulation: re-runs the true-margin waterfall
 * with one SKU's transactions removed. Powers the dashboard's silent-loser
 * insight (see getSilentLoserInsight in lib/engine.ts), which pairs this with
 * the underwriting model to show the limit impact of dropping the product.
 */
export function simulateExcludingSku(txs: Transaction[], skuToExclude: string): MarginResult {
  return aggregateTrueMargin(txs.filter((t) => t.sku !== skuToExclude));
}
...
export interface SkuMargin {
  ...
  /** returnsAllocated / grossRevenue across all grouped transactions (0–1). */
